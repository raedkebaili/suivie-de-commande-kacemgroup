...
export default function UsersView({ user: _ }: { user: User }) {
  ...
  return <div className="space-y-4">
    ...
    {show && <div className="fixed inset-0 z-50 flex items-center justify-center"><div className="absolute inset-0 bg-black/50" onClick={() => setShow(false)} /><div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6"><h4 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">{edit ? "Modifier" : "Nouvel Utilisateur"}</h4>{error && <div className="mb-3 p-2 rounded-lg bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm">{error}</div>}<div className="space-y-3"><F label="Nom d'utilisateur *" value={username} onChange={setUsername} /><F label="Mot de passe" value={password} onChange={setPassword} type="password" placeholder={edit ? "Laisser vide" : ""} /><F label="Nom complet *" value={fullName} onChange={setFullName} /><div><label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Rôle *</label><select value={role} onChange={e => setRole(e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-sm text-gray-700 dark:text-gray-200"><option value="superadmin">Super Admin</option><option value="commercial">Service Commercial</option><option value="technique">Service Technique</option><option value="planification">Service Planification</option><option value="consultant_prod">Consultant Prod</option></select></div></div><div className="flex justify-end gap-2 mt-6"><button onClick={() => setShow(false)} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg">Annuler</button><button onClick={save} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">{edit ? "Enregistrer" : "Créer"}</button></div></div></div>}
  </div>;
}
